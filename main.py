...
class Play2048s(ui.View):
    def __init__(self , comps:list[ui.Button], _2048:p_2048.Game, inter:Union[Interaction, commands.context.Context]):
    ...
    async def on_timeout(self) -> None:
        with open("best2048.json", "r") as f: j:dict = json.load(f)
        if type(self.inter) == Interaction:
            ...
            await self.inter.edit_original_message(view = None)
        with open("best2048.json", "w") as f: json.dump(j, f, indent = 4)
    ...
    async def callback(self, inter:Interaction):
        ...
        if m != 0:
            ...
            if self._type == "emj" or self._type == "emoji" or self._type == "이모지":
                with open("emoji2048.json", "r") as f: emoji2048:dict = json.load(f)
                _emj = self._2048.arr
                emj = ""
                for i in _emj:
                    for j in i:
...
@bot.slash_command(description = "2048을 플레이합니다", name = "2048")
...
@bot.slash_command(name = "2048랭크", description = "2048의 랭크를 확인합니다")
async def rank2048(inter:Interaction):
    try: await inter.response.defer()
    except: ...
    with open("best2048.json", "r") as f: _j:dict = json.load(f)
    ...
    isShow = False
    for rank in j[:10]:
        user = utils.get(bot.get_all_members() , id = int(rank[0]))
